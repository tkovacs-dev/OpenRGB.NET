...
namespace OpenRGB.NET.Test
{
    public class OpenRGBClientUnits : IDisposable
    {
        ...
        {
            Stopwatch sw = Stopwatch.StartNew();
            using (var client = new OpenRgbClient(name: "OpenRGB.NET Test: DisposePatternListController",
                       autoConnect: true))
            {
                int nbController = client.GetControllerCount();
                for (int i = 0; i < nbController; i++)
                ...
                for (int i = 0; i < devices.Length; i++)
                {
                    for (int j = 0; j < devices[i].Modes.Length; j++)
                    {
                        ...
                        if (mode.Flags.HasFlag(ModeFlags.HasModeSpecificColor) && mode.SupportsSpeed)
                        {
                            ...
                            client.UpdateMode(i, j, speed: mode.SpeedMax,
                                colors: Enumerable.Range(0, len).Select(_ => new Color(0, 255, 0)).ToArray());
                            break;
                        }
                    }
                }
        ...
        {
            using (var client = new OpenRgbClient(name: "OpenRGB.NET Test: LoadRandomProfile"))
            {
