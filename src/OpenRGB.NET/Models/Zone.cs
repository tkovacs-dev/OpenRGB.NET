...
namespace OpenRGB.NET
{
    ...
    public class Zone
    {
        private Zone(int index, int deviceIndex, string name, ZoneType type, uint ledCount, uint ledsMin, uint ledsMax,
            [CanBeNull] MatrixMap matrixMap, Segment[] segments)
        {
            Index = index;
        ...
        private static Zone ReadFrom(ref SpanReader reader, int deviceIndex, int zoneIndex,
            ProtocolVersion protocolVersion)
        {
            ...
            var segments = protocolVersion.SupportsSegmentsAndPlugins
                ? Segment.ReadManyFrom(ref reader, reader.ReadUInt16())
                : Array.Empty<Segment>();

            return new Zone(zoneIndex, deviceIndex, name, type, ledCount, ledsMin, ledsMax, matrixMap, segments);
        }

        internal static Zone[] ReadManyFrom(ref SpanReader reader, ushort zoneCount, int deviceID,
            ProtocolVersion protocolVersion)
        {
