...
namespace OpenRGB.NET.Utils
{
    ...
    internal ref struct SpanWriter
    {
        ...
        {
            BinaryPrimitives.WriteUInt16LittleEndian(Span.Slice(Position), value);
            Position += sizeof(ushort);
        }

        public void WriteInt32(int value)
        {
            BinaryPrimitives.WriteInt32LittleEndian(Span.Slice(Position), value);
            Position += sizeof(int);
        }

        public void WriteUInt32(uint value)
        {
            BinaryPrimitives.WriteUInt32LittleEndian(Span.Slice(Position), value);
            Position += sizeof(uint);
        }

        public void WriteBytes(ReadOnlySpan<byte> span)
        {
            span.CopyTo(Span.Slice(Position));
