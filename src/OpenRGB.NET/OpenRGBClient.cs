...
namespace OpenRGB.NET
{
    ...
    public sealed class OpenRgbClient : IDisposable, IOpenRgbClient
    {
            ...
            uint protocolVersionNumber = MAX_PROTOCOL_NUMBER)
        {
            ...
            foreach (var item in Enum.GetValues(typeof(CommandId)))
            {
                _pendingRequests[Convert.ToUInt32(item)] = new BlockingCollection<byte[]>();
            }
        ...
        private void SendMessage(CommandId command, ReadOnlySpan<byte> buffer = default, uint deviceId = 0)
        {
            ...
            if (_socket.Send(headerBuffer.ToArray()) != PacketHeader.Length)
        ...
        private byte[] SendMessageAndGetResponse(CommandId command, ReadOnlySpan<byte> buffer = default,
            uint deviceId = 0)
        {
            ...
            if (!_pendingRequests[(uint)command].TryTake(out var outBuffer, _timeoutMs))
                throw new TimeoutException(
                    $"Did not receive response to {command} in expected time of {_timeoutMs} ms");
        ...
        public Device GetControllerData(int id)
        {
            ...
            var response = SendMessageAndGetResponse(CommandId.RequestControllerData,
                BitConverter.GetBytes(CommonProtocolVersion.Number), (uint)id);
        ...
        private uint GetServerProtocolVersion()
        {
            ...
            try
            {
                ...
                serverVersion = BitConverter.ToUInt32(response, 0);
            }
        ...
        public void UpdateMode(int deviceId, int modeId,
            ...
            [CanBeNull] Color[] colors = default)
        {
            ...
            if (direction.HasValue)
            {
                if (!targetMode.SupportsDirection)
                    throw new InvalidOperationException(
                        "Cannot set direction on a mode that doesn't use this parameter");
